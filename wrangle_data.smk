...
nested_output=config['output_folder']+'/'+config['wrangler_dir']+'/'+config['analysis_dir']+'_run_stats'

rule all:
	input:
		output_configfile=nested_output+'/snakemake/config.yaml',
...
rule copy_files:
	...
	output:
		output_snakefile=nested_output+'/snakemake/wrangle_data.smk',
		output_configfile=nested_output+'/snakemake/wrangle_data.yaml',
		out_scripts=directory(nested_output+'/snakemake/scripts')
	shell:
		'''
...
rule reduce_sample_sheet:
	input:
		fastq_folder=config['fastq_dir'],
		in_ss=config['input_sample_sheet'],
	params:
		size=config['ss_reduction_size']
	output:
		out_ss=config['output_folder']+'/'+config['wrangler_dir']+'/top_'+str(config['ss_reduction_size'])+'_sample_sheet.tsv'
	script:
		'input_files/scripts/reduce_sample_sheet.py'
...
#rule generate_mip_files:
	...
	'''
#	input:
#		probe_file='placeholder_file.txt',
#		out_ss=config['output_folder']+'/'+config['wrangler_dir']+'/top_'+str(config['ss_reduction_size'])+'_sample_sheet.tsv'
#	output:
#		mip_arms=config['output_folder']+'/'+config['wrangler_dir']+'/mip_ids/mipArms.txt',
#		sample_file=config['output_folder']+'/'+config['wrangler_dir']+'/mip_ids/allMipsSamplesNames.tab.txt'
#	params:
#		probe_sets=config['probe_sets_used'],
#		sample_sets=config['sample_sets_used']
#	script:
